"""
With this tool, a video can be altered and single frames will be saved.

For this tool to work, cv2 is needed which can be installed using:
>>> pip install opencv-python

Other packages needed: numpy, pandas, math.
"""
...
def xy_coord(df):
    """
    Turn longitudes and latitudes into XY coordinates using an Equirectangular projection.
...
def orthorect_trans(img, M, C):
    """
    Image orthorectification based on parameters found with ip.orthorect_param().
